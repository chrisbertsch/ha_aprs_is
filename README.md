...
Connect Home Assistant to the [APRS-IS](http://www.aprs-is.net/) network (Automatic Packet Reporting System — Internet Service). Monitor positions, weather stations, and messages for your callsigns and any other amateur radio stations you want to track.

## Requirements

- Home Assistant 2026.1 or later
- A valid amateur radio callsign and APRS-IS passcode
- Use passcode `-1` for receive-only mode (outbound actions will be unavailable)
...
| Server | APRS-IS server (default: `rotate.aprs.net`) |
| Port | TCP port (default: `14580`) |

The login callsign is used only to authenticate the connection and receive inbound messages. All stations are configured separately in Options. To change these later, use **Reconfigure** from the integration's `⋮` menu.
...
| Option | Description |
|---|---|
| Range filter radius | Receive all packets within this many km of your HA location. `0` = disabled |
| Extra filter terms | Raw APRS-IS filter terms appended to the auto-built filter |
| Event rate limit | Max HA events fired per second. `0` = unlimited (default) |
| Position beacon interval | Transmit a position report for the login callsign every N minutes using the HA home location and a house (`/-`) symbol. `0` = disabled |
...
| Option | Description |
|---|---|
| WX beacon interval | Transmit every N minutes. `0` = disabled |
...
| Staleness sentinel entity | Optional. If this entity has not updated within the max age, the beacon is suppressed |
| Max data age | How stale the sentinel entity can be before the beacon is skipped |
